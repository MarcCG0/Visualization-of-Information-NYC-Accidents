import altair as alt 
import pandas as pd 
from colors import categorical_palette

df_V6 = pd.read_csv('../datasetsVisualizations/df_V6.csv')

chart_V6 = alt.Chart(df_V6).mark_bar().encode(
    alt.X('ACCIDENTS:Q', axis = alt.Axis(title = 'Accidents')),
    alt.Y('CONTRIBUTING FACTOR', sort = alt.EncodingSortField(field = 'ACCIDENTS', order = 'descending'), axis = alt.Axis(title = 'Contributing Factor')),
    color = alt.Color('YEAR:N', scale=alt.Scale(range=categorical_palette), legend = alt.Legend(title = 'Year'))
).properties(title = 'Accidents in NYC by Contributing Factor')

